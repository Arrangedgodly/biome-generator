...
A single centered column (max-width 1080px) holding one flexible main row: the specimen stage (width `min(90vmin, 640px)` — canvas, probe overlay, and the caption block stack inside it, aspect 1/1) beside a fixed 280px control column. The row never wraps; between ~641–975px the stage shrinks so tune-and-see stays side by side, and below 640px the layout stacks — stage first (100% up to 512px), controls below — remaining fully usable at 375px. Panel rhythm is three tiers: rows gap 0.5rem internally, the stack gaps 0.875rem, and cluster starts (presets, outputs) breathe 1.25rem — tune → act (presets + Generate) → export. Under the specimen, the stage label and the probe reading form one tight caption group (each reserving 1.5em so appearing/disappearing text never shifts the layout), closed by the biome legend — a native `<details>` collapsed to one muted "Biome legend" line until the visitor asks; its swatches are populated from the same `BIOMES` data the renderer classifies with, so the key can never drift from the pixels it explains. Touch targets are uniformly ≥ 2.5rem tall. The header carries one quiet exit ramp under the tagline: "View the source on GitHub" (Label size, Driftwood Muted, brightening on hover) — the portfolio visitor's evidence is the source.
...
Three quiet buttons in a content-based row (`flex: 1 1 auto`, Label-size 0.8125rem text, 0.25rem side padding) — Archipelago / Continent / Highlands, sized so the longest name holds one line inside the 280px panel. Active/pressed state spends two signals the quiet chips don't: the ocean-bright edge and full text color over the shared cut-out fill — persistent, and rendered from ONE source: the app's sticky preset id — the same value the share URL carries — never re-derived from slider positions. Chip, link, and truth cannot disagree: a moisture tweak keeps the chip pressed with the URL; an elevation input clears both together; returning to a preset's exact sliders does not re-press anything (coincidence is not selection).
...
- 512×512 internal, displayed up to 640px, aspect-locked 1/1, 10px radius, The Lamp shadow, `image-rendering: pixelated`, `touch-action: none` (touch drags survey, not scroll). Clicking it skips the staged animation — while the reveal plays the cursor switches from the probe's crosshair to `pointer` with a native "Click to skip the reveal" tooltip and `aria-busy`, so the skip affordance exists exactly when it is true. Its 12-color biome palette is *product data*, not chrome — the chrome borrows from it (the ocean accent), never the reverse.
...
- **Reading:** Driftwood Muted, 0.8125rem, tabular-nums, centered, reserved height (the stage-label discipline). Before the first probe the line is not blank — it teaches: "Hover the map to survey a pixel — arrow keys work too" (the empty state; any reading replaces it, and parking keeps a reading on the line, so it never returns after first use). The biome name is the classifier's *live* answer — `classify` under the controller's current moisture biases — so moving the moisture slider re-names the pixel under the crosshair.
...
Map status lives under the map, in the `#stage-status` caption block inside the stage — never in the control panel. The panel *drives* the stage label (reserved-height 1.5em, 0.8125rem, Driftwood Muted, centered) with full text written in JS — no CSS prefix — while the reveal runs ("Stage: Elevation → Moisture → Biomes"), settling to "Map ready · seed N" when done: the determinism promise stated in the caption, since N is exactly what a share link reproduces. Generate-class clicks (Generate, 🎲, presets, a committed new seed) fill the beat before the first frame with "Surveying…" — the busy cue, replaced by the first stage line and cleared by an error (the alert region owns the story then). The slider readouts are id-paired to their inputs (`aria-describedby`). The label is announced to assistive tech via aria-live (the visual line never blinks or jumps). It sits directly above the probe reading as one tight caption group.
